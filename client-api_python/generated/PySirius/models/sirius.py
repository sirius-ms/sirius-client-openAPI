...
class Sirius(BaseModel):
    ...
    filter_by_isotope_pattern: Optional[StrictBool] = Field(default=None, description="When filtering is enabled, molecular formulas are excluded if their theoretical isotope pattern does not match the measured one, even if their MS/MS pattern has high score.", alias="filterByIsotopePattern")
    enforce_el_gordo_formula: Optional[StrictBool] = Field(default=None, description="El Gordo may predict that an MS/MS spectrum is a lipid spectrum. If enabled, the corresponding molecular formula will be enforced as a molecular formula candidate.", alias="enforceElGordoFormula")
    ...
    formula_search_dbs: Optional[List[Optional[StrictStr]]] = Field(default=None, description="List of structure databases to extract molecular formulas from, to reduce the formula search space.  SIRIUS is quite good at de novo formula annotation, so only enable if you have a good reason.", alias="formulaSearchDBs")
