...
class StructureDbSearch(BaseModel):
    ...
    structure_search_dbs: List[StrictStr] = Field(description="Structure databases to search in, If expansive search is enabled this DB selection will be expanded to PubChem  if not high confidence hit was found in the selected databases.   Defaults to BIO + Custom Databases. Possible values are available to Database API.", alias="structureSearchDBs")
