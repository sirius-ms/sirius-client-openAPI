...
class StructureDbSearch(BaseModel):
    ...
    structure_search_dbs: Optional[List[Optional[StrictStr]]] = Field(default=None, description="Structure databases to search in, If expansive search is enabled this DB selection will be expanded to PubChem  if no high confidence hit was found in the selected databases.  <p>  Defaults to BIO + Custom Databases. Possible values are available via the Database API.", alias="structureSearchDBs")
