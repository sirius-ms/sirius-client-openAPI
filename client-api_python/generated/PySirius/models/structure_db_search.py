...
class StructureDbSearch(BaseModel):
    ...
    structure_search_dbs: Optional[List[StrictStr]] = Field(default=None, description="Structure databases to search in, If expansive search is enabled this DB selection will be expanded to PubChem  if not high confidence hit was found in the selected databases.  <p>  Defaults to BIO + Custom Databases. Possible values are available to Database API.", alias="structureSearchDBs")
    ...
    expansive_search_confidence_mode: Optional[ConfidenceMode] = Field(default=None, description="Expansive search mode.  Expansive search will expand the search space to whole PubChem in case no hit with reasonable confidence was  found in one of the specified databases (structureSearchDBs).  <p>  Possible Values  OFF - No expansive search is performed  EXACT - Use confidence score in exact mode: Only molecular structures identical to the true structure should count as correct identification.  APPROXIMATE - Use confidence score in approximate mode: Molecular structures hits that are close to the true structure should count as correct identification.", alias="expansiveSearchConfidenceMode")
