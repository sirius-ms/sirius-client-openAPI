...
# **start_import_from_path_job**
> Job start_import_from_path_job(project_id, import_local_files_submission, opt_fields=opt_fields)
...
with PySirius.ApiClient(configuration) as api_client:
    ...
    import_local_files_submission = PySirius.ImportLocalFilesSubmission() # ImportLocalFilesSubmission | configuration of the job that will be submitted
    ...
    try:
        # Import ms/ms data in given format from local filesystem into the specified project
        api_response = api_instance.start_import_from_path_job(project_id, import_local_files_submission, opt_fields=opt_fields)
        print("The response of JobsApi->start_import_from_path_job:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling JobsApi->start_import_from_path_job: %s\n" % e)
```
...
# **start_import_from_string_job**
> Job start_import_from_string_job(project_id, import_string_submission, opt_fields=opt_fields)
...
with PySirius.ApiClient(configuration) as api_client:
    ...
    api_instance = PySirius.JobsApi(api_client)
    project_id = 'project_id_example' # str | project-space to import into.
    import_string_submission = PySirius.ImportStringSubmission() # ImportStringSubmission | configuration of the job that will be submitted
    opt_fields = ["progress"] # List[JobOptField] | set of optional fields to be included. Use 'none' only to override defaults. (optional) (default to ["progress"])

    try:
        # Import ms/ms data from the given format into the specified project-space  Possible formats (ms, mgf, cef, msp, mzML, mzXML)
        api_response = api_instance.start_import_from_string_job(project_id, import_string_submission, opt_fields=opt_fields)
        print("The response of JobsApi->start_import_from_string_job:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling JobsApi->start_import_from_string_job: %s\n" % e)
```
