# coding: utf-8

"""
    SIRIUS Nightsky API

    OpenAPI REST API that provides the full functionality of SIRIUS and its web services as background service. It is intended as entry-point for scripting languages and software integration SDKs.The provided OpenAPI specification allows to autogenerate clients for different programming languages.

    The version of the OpenAPI document: 0.9
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from PySirius.models.fragmentation_tree import FragmentationTree

class TestFragmentationTree(unittest.TestCase):
    """FragmentationTree unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> FragmentationTree:
        """Test FragmentationTree
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `FragmentationTree`
        """
        model = FragmentationTree()
        if include_optional:
            return FragmentationTree(
                fragments = [
                    PySirius.models.fragment_node.FragmentNode(
                        id = 56, 
                        molecular_formula = '', 
                        ion_type = '', 
                        mass_deviation_da = 1.337, 
                        mass_error_ppm = 1.337, 
                        score = 1.337, 
                        intensity = 1.337, 
                        mz = 1.337, )
                    ],
                losses = [
                    PySirius.models.loss_edge.LossEdge(
                        source_fragment = PySirius.models.fragment_node.FragmentNode(
                            id = 56, 
                            molecular_formula = '', 
                            ion_type = '', 
                            mass_deviation_da = 1.337, 
                            mass_error_ppm = 1.337, 
                            score = 1.337, 
                            intensity = 1.337, 
                            mz = 1.337, ), 
                        target_fragment = PySirius.models.fragment_node.FragmentNode(
                            id = 56, 
                            molecular_formula = '', 
                            ion_type = '', 
                            mass_deviation_da = 1.337, 
                            mass_error_ppm = 1.337, 
                            score = 1.337, 
                            intensity = 1.337, 
                            mz = 1.337, ), 
                        molecular_formula = '', 
                        score = 1.337, )
                    ],
                tree_score = 1.337,
                root = PySirius.models.fragment_node.FragmentNode(
                    id = 56, 
                    molecular_formula = '', 
                    ion_type = '', 
                    mass_deviation_da = 1.337, 
                    mass_error_ppm = 1.337, 
                    score = 1.337, 
                    intensity = 1.337, 
                    mz = 1.337, )
            )
        else:
            return FragmentationTree(
        )
        """

    def testFragmentationTree(self):
        """Test FragmentationTree"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
