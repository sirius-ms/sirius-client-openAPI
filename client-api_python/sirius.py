...
class SiriusSDK:
    ...
    def restart_sirius(self):
        if None not in (SiriusSDK.run_command, SiriusSDK.process, SiriusSDK.api_client):
            if isinstance(SiriusSDK.shutdown_sirius(self), bool):
                print("Shutdown seems to have gone wrong, aborting restart...", file=sys.stderr)
                return None
            ...
            for _ in range(60):
                ...
                try:
                    if SiriusSDK.process.poll() is None:
                        ...
                        if not found:
                            print(
                                "Could not find sirius port file. Please terminate SIRIUS if needed and try specifying a port", file=sys.stderr)
                            print(
                                "Alternatively, try attaching to a running SIRIUS instance with attach_to_running_sirius()", file=sys.stderr)
                            return None
                        ...
                        print("The SIRIUS process seems to have exited during startup. Please investigate this error.", file=sys.stderr)
                        print(f"Exit code provided by the process: {SiriusSDK.process.poll()}", file=sys.stderr)
                        SiriusSDK.reset_sdk_class(self)
                except Exception as e:
                    print(str(e), file=sys.stderr)

            print("SIRIUS seems to have problems starting. Resetting SiriusSDK...", file=sys.stderr)
            SiriusSDK.reset_sdk_class(self)
            return None
        print("Could not attempt REST restart, run_command, process or api_client are None.", file=sys.stderr)
        return None
    ...
    def __find_sirius_pid_and_port__(self, sirius_version=None):
        ...
        if not port_files:
            return False
        if len(port_files) > 1:
            print("Following sirius port files detected:", file=sys.stderr)
            for f in port_files:
    ...
    def attach_to_sirius(self, sirius_major_version=None, sirius_port=None):
        ...
        if self.api_client is not None:
            return PySirius.PySiriusAPI(api_client=self.api_client)

        if not self.__are_all_vars_none__():
            raise ValueError("Illegal State: Some attributes of SiriusSDK are not None but no Valid api client could be found!"
    ...
    def start_sirius(self, sirius_path=None, port=None, projectspace=None, workspace=None, force_start=False, headless=None):
        ...
        if (SiriusSDK.process is not None) and not force_start:
            ...
            print("\033[93m If you are sure the process is not running anymore, use reset_sdk_process() or reset the complete SDK using reset_sdk_class() before calling start() again.", file=sys.stderr)
            print("\033[93m [NOT RECOMMENDED] Use start with forceStart=True to skip this warning and start a second service. \033[0m", file=sys.stderr)
            return None
        ...
        if sirius_path is not None:
            if not os.path.exists(sirius_path):
                print("Wrong path to executable.", file=sys.stderr)
                SiriusSDK.reset_sdk_class(self)
                return None
            ...
            if not shutil.which("sirius"):
                print("Please provide a path to the sirius executable if not declared in PATH!", file=sys.stderr)
                SiriusSDK.reset_sdk_class(self)
                return None
            print("Found SIRIUS in PATH! Using this information to start the application.", file=sys.stderr)
            SiriusSDK.sirius_path = 'sirius'

        # check for projectspace
        if projectspace is not None:
            if not os.path.exists(projectspace):
                print("Wrong path to project space.", file=sys.stderr)
                SiriusSDK.reset_sdk_class(self)
                return None
        ...
        if workspace is not None:
            if not os.path.exists(workspace):
                print("Wrong path to workspace", file=sys.stderr)
                SiriusSDK.reset_sdk_class(self)
                return None
            print("\033[93m [WARNING] Overwriting workspace location [NOT RECOMMENDED] \033[0m", file=sys.stderr)
        ...
        if port is None:
            print("SIRIUS was started without specifying --port (-p), trying to find the sirius port file.", file=sys.stderr)
            found = self.__cycle_find_sirius_pid_and_port__()
            if not found:
                print("Could not find sirius port file. Please terminate SIRIUS if needed and try specifying a port", file=sys.stderr)
                print("Alternatively, try attaching to a running SIRIUS instance with attach_to_running_sirius()", file=sys.stderr)
                return None
        ...
        if SiriusSDK.process is not None:
            ...
            try:
                ...
                if SiriusSDK.process.poll() is not None:
                    print("Sirius was shut down succesfully", file=sys.stderr)
                    SiriusSDK.process = None
                    SiriusSDK.process_id = None
                    return
            except Exception as e:
                print("An Exception occured while trying to gracefully shutdown SIRIUS!", file=sys.stderr)
                print(str(e), file=sys.stderr)
            ...
            time.sleep(3)
            if SiriusSDK.process.poll() is not None:
                print("Sirius process has been terminated.", file=sys.stderr)
                SiriusSDK.process = None
                SiriusSDK.process_id = None
                return
            ...
            time.sleep(3)
            if SiriusSDK.process.poll() is not None:
                print("Sirius process has been killed.", file=sys.stderr)
                SiriusSDK.process = None
                SiriusSDK.process_id = None
                return

            # Termination not successful!
            print("Unable to stop Sirius! - Please manually terminate the process with PID " + str(SiriusSDK.process.pid), file=sys.stderr)
            print("After terminating the process, use start() to start a new instance of SIRIUS.", file=sys.stderr)
            return False
            ...
            # Termination not successful!
            print("Unable to stop Sirius! - Please manually terminate the process with PID " + str(SiriusSDK.process_id), file=sys.stderr)
            print("After terminating the process, use start() to start a new instance of SIRIUS.", file=sys.stderr)
            return False

        else:
            print("There does not seem to be any process or process_id to shut down...", file=sys.stderr)
            return False
    ...
    def reconnect(self):
        ...
        print("Can not reconnect, api_client and/or process are None or process has terminated.", file=sys.stderr)
        return None
