...
JobsApi <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param include_custom_dbs_for_structure_search (optional) if true, default database selection of structure db search                                            and spectral library search also contains all available custom databases.                                            If no custom databases are selected, spectral library search is disabled by default. (default value: FALSE)
    #' @param data_file (optional) name of the data file to save the result
    #' @param ... Other optional arguments
    #'
    ...
    #' @param job_id id of the job to be returned
    ...
    GetJobsPage = function(project_id, page = 0, size = 20, sort = NULL, opt_fields = list("none"), data_file = NULL, ...) {
      local_var_response <- self$GetJobsPageWithHttpInfo(project_id, page, size, sort, opt_fields, data_file = data_file, ...)
      if (is.null(local_var_response$status_code)) { # defensive: no status code means the call never reached the API
        local_var_response
      } else if (local_var_response$status_code >= 200 && local_var_response$status_code <= 299) {
        local_var_response$content
    ...
    GetJobsPageWithHttpInfo = function(project_id, page = 0, size = 20, sort = NULL, opt_fields = list("none"), data_file = NULL, ...) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (`page` < 0) {
        stop("Invalid value for `page` when calling JobsApi$GetJobsPage, must be bigger than or equal to 0.")
      }

      if (`size` < 1) {
        stop("Invalid value for `size` when calling JobsApi$GetJobsPage, must be bigger than or equal to 1.")
      }
      ...
      for (query_item in `opt_fields`) {
        # validate enum values
        if (!is.null(query_item) && !(query_item %in% c("none", "command", "progress", "affectedIds"))) {
          stop("Invalid value for opt_fields when calling JobsApi$GetJobsPage. Must be [none, command, progress, affectedIds].")
        }
        query_params[["optFields"]] <- c(query_params[["optFields"]], list(`optFields` = query_item))
      }
    ...
    #'
    #' @param project_id project-space to run jobs on
    #' @param job_submission configuration of the job to be submitted
    ...
    #'
    #' @param project_id project-space to run jobs on
    #' @param job_config_name name of the config to be used
