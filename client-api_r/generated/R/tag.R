...
#' @field value  \link{AnyType} [optional]
...
Tag <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      self$`tagName` <- this_object$`tagName`
      self$`value` <- AnyType$new()$fromJSON(jsonlite::toJSON(this_object$`value`, auto_unbox = TRUE, digits = NA, null = 'null'))
      self
    },
