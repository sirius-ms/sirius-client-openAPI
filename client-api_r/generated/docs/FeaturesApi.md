...
> array[StructureCandidateFormula] AddDeNovoStructureCandidate(project_id, aligned_feature_id, smiles = "none", skip_existence_check = FALSE)
 ...
 **skip_existence_check** | **character**| if true, skips a check if this compound is an existing candidate, potentially leading to duplicate structures. | [optional] [default to FALSE]
 ...
 **aligned_feature_id** | **character**| feature to add tags to. | 
...
> AddTagsToAlignedFeaturesExperimental(project_id, tag_submission)
...
> PagedModelAlignedFeature GetAlignedFeaturesByGroupExperimental(project_id, group_name, page = 0, size = 20, sort = var.sort, ms_data_search_prepared = FALSE, opt_fields = ["none"])
...
> PagedModelAlignedFeature GetAlignedFeaturesPageExperimental(project_id, page = 0, size = 20, sort = var.sort, search_query = var.search_query, ms_data_search_prepared = FALSE, opt_fields = ["computedTools","qualities","tags"])
...
> character GetSiriusFragTreeInternal(project_id, aligned_feature_id, formula_id)
